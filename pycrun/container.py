...
https://tmsyscall.readthedocs.io/en/latest/Example.html
...
from .colorhelper import print_info, print_list
from .network import set_loopback, set_container_veth, set_host_veth
...
def child(rootfs_path, cmd, container_id, container_options):
...
def runc(rootfs_path, command, container_options):
    """Run a command in a new container"""
    resolv__path = Path(rootfs_path).joinpath("etc/resolv.conf")
    if resolv__path.is_symlink():
        resolv__path.unlink();
    shutil.copyfile("/etc/resolv.conf", resolv__path)    
    container_id = str(uuid4())
    ...
    pid = os.fork()
    if pid == 0:
        child(rootfs_path, command, container_id, container_options)
        exit(0)
    return parent(pid)
